...
public interface TicketingRepository extends CrudRepository<Ticket,Long> {
    ...
    @Query(value = "SELECT t.id FROM ticket t,ticketdetails td WHERE  td.ticket_id = t.id and td.train_id =?1 and t.booking_date = ?2  ", nativeQuery = true)
	public  List<BigInteger>  findAllByTrainId(@Param("train_id") long train_id,@Param("booking_date") long booking_date);
